...
def pytest_addoption(parser):
    parser.addoption(
        "--external", action="store_true", default=False, help="run tests which call external code"
    )
    parser.addoption(
        "--tccloud",
        action="store_true",
    ...
    parser.addoption("--full",action="store_true",default=False,help="run full optimization cycle tests")
    parser.addoption("--gpu",action="store_true",default=False,help="run tests which require gpus")
    parser.addoption("--queue",action="store_true",default=False,help="run tests which use the queue")
...
def pytest_configure(config):
    ...
    config.addinivalue_line("markers", "full: test does a full optimization cycle and is very slow to run")
    config.addinivalue_line("markers", "gpu: test requires a gpu to run")
    config.addinivalue_line("markers", "queue: test runs on queue and may take long to run")
...
def pytest_collection_modifyitems(config, items):
    ...
    for item in items:
        for keyword in skips.keys():
            if not config.getoption(f"--{keyword}") and keyword in item.keywords and not config.getoption("--all"):
