import pytest
from ff_optimizer import active_learning, utils, model
import numpy as np
import os
...
def test_collectGeometries(monkeypatch):
    os.chdir(os.path.join(os.path.dirname(__file__), "active_learning", "collectGeometries"))
    monkeypatch.setattr(active_learning.ActiveLearningModel,"__init__",monkeyInit)
    args = FakeArgs()
    model = active_learning.ActiveLearningModel(args)
...
def test_chooseGeometries3Models(monkeypatch):
    monkeypatch.setattr(active_learning.ActiveLearningModel,"__init__",monkeyInit)
    os.chdir(os.path.join(os.path.dirname(__file__), "active_learning", "chooseGeometries"))
    args = FakeArgs()
    model = active_learning.ActiveLearningModel(args)
    energies = []
    forces = []
...
def test_doActiveLearning(monkeypatch):
    monkeypatch.setattr(active_learning.ActiveLearningModel,"__init__",monkeyInit)
    monkeypatch.setattr(active_learning.ActiveLearningModel,"computeAll",monkeyComputeAll)
    monkeypatch.setattr(active_learning.ActiveLearningModel,"chooseGeometries",monkeyChooseGeometries)
    os.chdir(os.path.join(os.path.dirname(__file__), "active_learning", "doActiveLearning"))
    args = FakeArgs()
    model = active_learning.ActiveLearningModel(args)
    for i in range(1,4):
        for folder in ["train", "valid_1", "valid_2"]:
            copyfile(os.path.join("ref",f"{i}.pdb"), os.path.join(f"model_{i}", "2_sampling", "7_cycle_7", folder, "1.pdb"))

    model.doActiveLearning(7)
    testGeom = utils.readPDB(os.path.join("model_1", "2_sampling", "7_cycle_7", "train", "1.pdb"))
    refGeom = utils.readPDB(os.path.join("ref", "1.pdb"))
    checkUtils.checkArray(testGeom, refGeom)
    testGeom = utils.readPDB(os.path.join("model_2", "2_sampling", "7_cycle_7", "valid_2", "1.pdb"))
    checkUtils.checkArray(testGeom, refGeom)
    testGeom = utils.readPDB(os.path.join("model_3", "2_sampling", "7_cycle_7", "valid_1", "1.pdb"))
    checkUtils.checkArray(testGeom, refGeom)

    testGeom = utils.readPDB(os.path.join("model_2", "2_sampling", "7_cycle_7", "train", "1.pdb"))
    refGeom = utils.readPDB(os.path.join("ref", "2.pdb"))
    checkUtils.checkArray(testGeom, refGeom)
    testGeom = utils.readPDB(os.path.join("model_3", "2_sampling", "7_cycle_7", "valid_2", "1.pdb"))
    checkUtils.checkArray(testGeom, refGeom)
    testGeom = utils.readPDB(os.path.join("model_1", "2_sampling", "7_cycle_7", "valid_1", "1.pdb"))
    checkUtils.checkArray(testGeom, refGeom)

    testGeom = utils.readPDB(os.path.join("model_3", "2_sampling", "7_cycle_7", "train", "1.pdb"))
    refGeom = utils.readPDB(os.path.join("ref", "3.pdb"))
    checkUtils.checkArray(testGeom, refGeom)
    testGeom = utils.readPDB(os.path.join("model_1", "2_sampling", "7_cycle_7", "valid_2", "1.pdb"))
    checkUtils.checkArray(testGeom, refGeom)
    testGeom = utils.readPDB(os.path.join("model_2", "2_sampling", "7_cycle_7", "valid_1", "1.pdb"))
    checkUtils.checkArray(testGeom, refGeom)
