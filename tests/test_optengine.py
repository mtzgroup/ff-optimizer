...
def test_readOpt3():
    ...
    assert status == 0
    refParams = [-6.5120e-01, 3.2560e-01, 1.0000e+02, 1.0452e+02, 1.7564e+00, 2.9777e-01]
    assert checkUtils.checkArray(refParams, results['params'])
    refInitialParams = [-9.4824e-01, 4.7412e-01, 1.0000e+02, 1.0452e+02, 1.7564e+00, 1.7912e-01]
    assert checkUtils.checkArray(refInitialParams, results['initialParams'])
    refLabels = ['COUL:SOL-1', 'COUL:SOL-2', 'ANGLESK/HW-OW-HW', 'ANGLESB/HW-OW-HW', 'VDWS/OW', 'VDWT/OW']
...
def test_readValid2():
    ...
    cleanOptDir(options['optdir'])
    try:
        test = optEngine.readValid("validFail.out")
        assert False
...
def monkeyForceBalance(command):
    ...
    if not os.path.isfile(os.path.join("ref",out)):
        raise FileNotFoundError(errno.ENOENT, os.strerror(errno.ENOENT), os.path.join("ref",out))
...
def test_optimizeForcefield1(monkeypatch):
    ...
    copyfile(os.path.join("ref","dasa_prev.mol2"),os.path.join("result","opt_3","dasa.mol2"))
    copyfile(os.path.join("ref","dasa_prev.frcmod"),os.path.join("result","opt_3","dasa.frcmod"))
...
def test_sortParams1():
    ...
    refParams = [2.6650e+02, 1.6724e+02, 1.9135e+02, 2.1897e+02, 2.3580e+02]
    checkUtils.checkArray(refParams, optEngine.params[:5,0])
    refParams = [-6.0860e+00,-5.3713e+00,-4.3450e+00,-4.5274e+00,-4.4187e+00]
...
def test_respPriors(monkeypatch):
    ...
    assert checkUtils.checkFileFloatsNoWhitespace("opt_4.in", os.path.join("ref","opt_4_priors.in"))

    assert checkUtils.checkFileFloatsNoWhitespace("prev_dasa.mol2",os.path.join("ref","dasa_priors.mol2"))
    os.chdir("..")
    cleanOptDir(options['optdir'])
