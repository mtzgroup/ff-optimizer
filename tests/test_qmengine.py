...
class TestQMEngine:
    ...
    def test_writeInputFileResp(self):
        ...
        readSettings = qmEngine.readInputFile(testPath)
        os.remove(testPath)
        assert "coordinates coors.xyz" in lines[0]
        assert "run gradient" in lines[1]
        assert "resp yes" in lines[-1]
        assert readSettings == inputSettings
    ...
    def test_writeFBdata(self):
        ...
        for i in range(len(refLines)):
            ...
            for j in range(len(refLine)):
                try:
                    assert checkUtils.checkFloats(refLine[j], testLine[j], 0.0001)
                except:
    ...
    def test_writeResult(self):
        ...
        os.chdir(os.path.dirname(__file__))
        refResult = SinglePointResults.open(os.path.join("qmengine", "tc_1_ref.json"))
        qmEngine.writeResult(
            os.path.join("qmengine", "tc_1.out"), os.path.join("qmengine", "1.xyz")
        )
        testResult = SinglePointResults.open(os.path.join("qmengine", "tc_1.json"))
