...
from qccloud.models import AtomicInput, AtomicResult, to_file, from_file, FailedOperation
from qccloud.exceptions import TimeoutError
import traceback
...
class TCCloudEngine(QMEngine):
        ...
        for pdb in sorted(pdbs):
            ...
            if self.doResp:
                atomicInput = AtomicInput(
                    ...
                    extras={"tcfe:keywords": {"native_files": ["esp.xyz"], "stdout" : True}},
                )
            else:
                atomicInput = AtomicInput(
                    molecule=mol,
    ...
    def runJobs(self, pdbs: list, useBackup=False):
        ...
        if len(results) != len(atomicInputs):
            raise RuntimeError("TCCloud did not return the same number of results as inputs")
        retryPdbs = []
