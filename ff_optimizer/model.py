...
class Model:
    ...
    def doMMSampling(self, i):
        ...
        for f in os.listdir(os.path.join(self.optdir, "result", f"opt_{str(i-1)}")):
            copyfile(os.path.join(self.optdir, "result", "opt_" + str(i - 1),f), os.path.join(samplePath, f))
    ...
    def doParameterOptimization(self, i):
        ...
        for f in os.listdir(os.path.join(self.sampledir, f"{str(i)}_cycle_{str(i)}")):
            if f.startswith("train_") and os.path.isdir(
                ...
                mmTrainFolder = os.path.join(self.sampledir, f"{str(i)}_cycle_{str(i)}", f)
            elif f.startswith("valid_") and os.path.isdir(
                os.path.join(self.sampledir, f"{str(i)}_cycle_{str(i)}", f)
            ):
                valids.append(os.path.join(self.sampledir, f"{str(i)}_cycle_{str(i)}", f))
