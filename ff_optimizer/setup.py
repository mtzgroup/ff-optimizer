...
class Setup():
    def __init__(self, xyz, charge=0):
        self.xyz = Path(xyz).absolute()
        readXYZ(self.xyz)  # easy way to check that xyz is formatted correctly and exists
        checkForAmber()
        self.home = Path(".").absolute()
        self.charge = charge
        self.spinMult = 1 # currently hard-coded, would be a minor pain to determine automatically
        self.makeInput()
    ...
    def setupOpt(self):
        ...
        if charge != self.charge:
            raise ValueError(f"Specified charge {self.charge} does not match charge {charge} in mol2 file")
    ...
    def writeTCFiles(self):
        ...
        print(f"Defaulting to wB97X-D3/def2-svp and a spin multiplicity of {self.spinMult}")
    ...
    def setupFF(self):
        ...
        output = subprocess.check_output(["antechamber", "-i", pdb, "-fi", "pdb", "-o", mol2, "-fo", "mol2", "-c", "bcc", "-nc", str(self.charge)])
    ...
    def setupForceBalance(self, frcmod, mol2, resname):
