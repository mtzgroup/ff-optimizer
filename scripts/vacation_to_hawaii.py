...
parser.add_argument('--dynamicsdir',help="Folder containing TC output file from MD trajectory and XYZ coord file from MD trajectory, default is 0_dynamics", type=str, default="0_dynamics")
parser.add_argument('--tcout', help="TeraChem output file from MD trajectory, default is tc.out", type=str, default="tc.out")
parser.add_argument('--coors', help="XYZ file containing coords from TeraChem MD trajectory, default is coors.xyz", type=str, default="coors.xyz")
parser.add_argument('--start', help="Lower bound for sampling of frames from MD trajectory, default is the first frame", type=int, default=None)
parser.add_argument('--end', help="Upper bound for sampling of frames from MD trajectory, default is the last frame", type=int, default=None)
parser.add_argument('--stride', help="Stride between successive samples of frames from MD trajectory, default is 50", type=int, default=50)
...
parser.add_argument('--maxcycles', help="Maximum number of optimization/sampling cycles to be performed, default is 10",type=int,default=10)
parser.add_argument('--sampledir', help="Directory where MM sampling is performed, default is 2_sampling",type=str,default="2_sampling")
parser.add_argument('--split', help='If set, sample one geometry from before this frame and one from after', type=int, default=None)
parser.add_argument('--opt0', help="ForceBalance input file for first optimization, with a single target named dynamics, default is opt_0.in", type=str, default="opt_0.in")
parser.add_argument('--valid0', help="Template ($options section only) for ForceBalance input files for validation single points, default is valid_0.in", type=str, default="valid_0.in")
parser.add_argument('--engine', help="Engine for performing QM calculations, currently supported is fire", type=str, default="fire")
parser.add_argument('--sbatch', help="Sbatch template file for submitting jobs to fire, default is sbatch_template.sh", type=str, default="sbatch_template.sh")
parser.add_argument('--tctemplate', help="TC input template file for submitting jobs to fire, default is tc_template.in", type=str, default="tc_template.in")
parser.add_argument('--tctemplate_long', help="TC input template file for resubmitting failed jobs to fire, default is tc_template_long.in", type=str, default="tc_template_long.in")
parser.add_argument('--restart', help="Restart partially complete forcefield optimization", action="store_true")
...
if restartCycle < 0:
    ...
    if not os.path.isfile(os.path.join(args.dynamicsdir,args.coors)):
        raise RuntimeError('XYZ coordinates (from QM dynamics) ' + args.coors + ' does not exist in ' + args.dynamicsdir)
    if not os.path.isfile(os.path.join(args.dynamicsdir,args.tcout)):
        raise RuntimeError('TC output file (from QM dynamics ' + args.tcout + ' does not exist in ' + args.dynamicsdir)
    if not os.path.isdir(args.optdir):
        raise RuntimeError('ForceBalance optimization directory ' + args.optdir  + ' does not exist')
    if not os.path.isfile(os.path.join(args.optdir,"conf.pdb")):
        raise RuntimeError('Prototype PDB coordinates conf.pdb does not exist in ' + args.optdir)
    ...
    if not os.path.isfile(os.path.join(args.optdir,args.opt0)):
        raise RuntimeError('Initial ForceBalance optimization input file ' + args.opt0 + ' does not exist in ' + args.optdir)
    if not os.path.isfile(os.path.join(args.optdir,args.valid0)):
        raise RuntimeError('Initial ForceBalance validation input file ' + args.valid0 + ' does not exist in ' + args.optdir)
    ...
    if not os.path.isfile(os.path.join(args.sampledir,"heat1.in")):
        raise RuntimeError('No sander input file for equilibration named heat1.in provided in ' + args.sampledir)
    if not os.path.isfile(os.path.join(args.sampledir,"md.in")):
        raise RuntimeError('No sander input file for sampling named md.in provided in ' + args.sampledir)
    ...
    if args.engine == "fire":
        if not os.path.isfile(os.path.join(args.sampledir,args.sbatch)):
            raise RuntimeError('Sbatch template ' + args.sbatch + ' does not exist in ' + args.sampledir)
        if not os.path.isfile(os.path.join(args.sampledir,args.tctemplate)):
            raise RuntimeError('TC input template ' + args.tctemplate + ' does not exist in ' + args.sampledir)
        if not os.path.isfile(os.path.join(args.sampledir,args.tctemplate_long)):
            raise RuntimeError('TC input template (for resubmitting jobs) ' + args.tctemplate_long + ' does not exist in ' + args.sampledir)
...
if restartCycle < 0:
    if not os.path.isfile(os.path.join(args.optdir,mol2)):
        raise RuntimeError('Mol2 ' + mol2 + ' specified in ' + args.opt0 + ' is not in ' + args.optdir)
    if not os.path.isfile(os.path.join(args.optdir,frcmod)):
        raise RuntimeError('Frcmod ' + frcmod + ' specified in ' + args.opt0 + ' is not in ' + args.optdir)
    ...
    with open(os.path.join(args.optdir, args.valid0), 'r') as srcValid:
        with open(os.path.join(args.optdir, "valid_0_initial.in"), 'w') as destValid:
            for line in srcValid.readlines():
                destValid.write(line.replace(frcmod,"initial_"+frcmod).replace(mol2,"initial_"+mol2))
                if "$target" in line:
                    break
...
if restartCycle < 0:
    ...
    if not os.path.isdir(path):
        os.mkdir(path)
    l = convertTCtoFB(os.path.join(args.dynamicsdir,args.tcout), os.path.join(args.dynamicsdir,args.coors), args.stride, args.start, args.end, os.path.join(path,"qdata.txt"), os.path.join(path,"all.mdcrd"))
...
os.rename(os.path.join(args.optdir,args.valid0),os.path.join(args.optdir,"valid_0.in"))
...
for i in range(1,args.maxcycles+1):
    ...
    for f in files:
        ...
        if ".frcmod" in f:
            frcmod = f
    if prmtop == None:
        raise RuntimeError("Tleap failed to create a new .prmtop file, check " + os.path.join(samplePath,"leap.out") + " for more information")
    ...
    for rst in rsts:
        ...
        if not os.path.isfile(name + "_sample.nc"):
            ...
            for j in range(1,heatCounter+1):
                os.system("sander -O -p " + prmtop + " -i heat" + str(j) + ".in -o " + name + "_heat" + str(j) + ".out -c " + name + "_heat" + str(j-1) + ".rst7 -x " + name + "_heat" + str(j) + ".nc -r " + name + "_heat" + str(j) + ".rst7")
            os.system("sander -O -p " + prmtop + " -c " + name + "_heat" + str(heatCounter) + ".rst7 -x " + name + "_sample.nc -v " + name + "_vel.nc -i md.in -o " + name + ".out")
        ...
        if not QMFinished:
            if args.engine == "fire":        
                ...
                for j in jobs:
                    ...
                    os.system("sed 's/XXX/" + str(j) + "/g' ../../" + args.sbatch + " > " + submit)
                    os.system("sed 's/XXX/" + str(j) + "/g' ../../" + args.tctemplate + " > tc_" + str(j) + ".in")
                    os.system("sed 's/XXX/" + str(j) + "/g' ../../" + args.tctemplate_long + " > tc_" + str(j) + "_long.in")
                ...
                for j in range(1,batchSize+1):
                    ...
                    if grads == -1:
                        ...
                        if os.path.isfile("qdata.txt"):
                            os.remove("qdata.txt")
                        raise RuntimeError("TC job " + name + "/" + fileName + " did not complete")
                    with open("qdata.txt",'a') as f:
                        f.write("JOB " + str(j) + "\n")
                ...
                for j in jobs:
                    ...
                    if grads == -1:
                        ...
                        if grads == -1:
                            ...
                            if os.path.isfile("qdata.txt"):
                                os.remove("qdata.txt")
                            raise RuntimeError("TC job " + name + "/" + out + " did not complete")
                    pdb = str(j) + ".pdb"
                    coords = readPDB(pdb)
                    with open("qdata.txt",'a') as f:
                        f.write("JOB " + str(j) + "\n")
    ...
    if args.split is not None:
        if rstNumber < args.split:
            src = os.path.join(args.sampledir,str(i) + "_cycle_" + str(i),str(rstNumber),"all.mdcrd")
            dest = os.path.join(args.optdir,"targets","train_" + str(i),".")
            os.system(f"cp {src} {dest}")
            src = os.path.join(args.sampledir,str(i) + "_cycle_" + str(i),str(rstNumber),"qdata.txt")
            os.system(f"cp {src} {dest}")
            src = os.path.join(args.sampledir,str(i) + "_cycle_" + str(i),str(rstNumber2),"all.mdcrd")
            dest = os.path.join(args.optdir,"targets","valid_" + str(i),".")
            os.system(f"cp {src} {dest}")
            src = os.path.join(args.sampledir,str(i) + "_cycle_" + str(i),str(rstNumber2),"qdata.txt")
            os.system(f"cp {src} {dest}")
        else:
            src = os.path.join(args.sampledir,str(i) + "_cycle_" + str(i),str(rstNumber),"all.mdcrd")
            dest = os.path.join(args.optdir,"targets","valid_" + str(i),".")
    ...
    addTargetLines(os.path.join(args.optdir, "opt_" + str(i) + ".in"),targetLines,initialTarget,"train_" + str(i))
    addTargetLines(os.path.join(args.optdir, "valid_" + str(i) + ".in"),targetLines,initialTarget,"valid_" + str(i))
    addTargetLines(os.path.join(args.optdir, "valid_" + str(i) + "_initial.in"),validInitialTargetLines,initialTarget,"valid_" + str(i))
    ...
    if len(validPrevious) <= i:
        if i > 1:
            ...
            dest = os.path.join(args.optdir,"forcefield")
            os.system(f"cp {src} {dest}")
        os.system("ForceBalance.py valid_" + str(i) + ".in > valid_" + str(i) + "_previous.out")
    ...
    if (len(train)) <= i:
        ...
        if status == -1:
            raise RuntimeError("ForceBalance optimization of " + os.path.join(args.optdir,"opt_" + str(i) + ".in") + " failed")
        if status == 1:
    ...
    if len(validInitial) <= i:
        os.system("ForceBalance.py valid_" + str(i) + "_initial.in > valid_" + str(i) + "_initial.out")
