...
def runValid(optdir, inp, out, parmFolder, frcmod, mol2):
    ...
    copyfile(parmFolder / frcmod, optdir / "targets" / frcmod)
    copyfile(parmFolder / mol2, optdir / "targets" / mol2)
    os.chdir(optdir)
...
def getFinalValidations(i, lastCycle, optdir="1_opt", patience=5):
    vs = []
    ...
    for j in range(lastCycle-patience, lastCycle + 1):
        ...
        try:
            vs.append(readValid(out))
        except:
            runValid(optdir, inp, out, f"{optdir}/result/opt_{j}", "dasa.frcmod", "dasa.mol2")
            vs.append(readValid(Path(optdir) / out))
    vs = np.asarray(vs)
    return np.argmin(vs) + 1 # account for one-indexing of validations
